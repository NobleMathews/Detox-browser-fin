            ...
            for (i in wordlist) {
                if (wordlist[i] != '' && wordlist[i].slice(0, 2) != '//') {
                    ...
                    if (new RegExp(word, 'gi')
                        .test($(this).text().replace(/[&\/\\#,+()$~%.'":*?<>{}]/g, '')) && $(this).css('display') != 'none') {
                        hideElem($(this), word);
                        superSet.add(word);
                    }
                    else 
                    if(abusingOption)
                    if(checkall($(this).text().replace(/[&\/\\#,+()$~%.'":*?<>{}]/g, '')) && $(this).css('display') != 'none'){
    ...
    if(superSet.size>0){
        if(!$('#blacklist_blocker').length){
            $("body").append(
                ...
                <p>Are you sure you want to continue ?? This page discusses about blacklisted topics which are : <b>`+[...superSet].join(', ')+`</b><button style="display:block"} onclick="window.history.go(-1)">Back</button></p></div>`
